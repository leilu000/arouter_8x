...
import com.alibaba.android.arouter.register.core.RegisterTransform11
...
class ScanUtil11 {
    ...
    static void scanJar(File jarFile, File destFile) {
        if (jarFile) {
            ...
            while (enumeration.hasMoreElements()) {
                ...
                if (entryName.startsWith(ScanSetting11.ROUTER_CLASS_PACKAGE_NAME)) {
                    InputStream inputStream = file.getInputStream(jarEntry)
                    scanClass(inputStream, entryName)
                    inputStream.close()
                } else if (ScanSetting11.GENERATE_TO_CLASS_FILE_NAME == entryName) {
                    // mark this jar file contains LogisticsCenter.class
                    // After the scan is complete, we will generate register code into this file
                    RegisterTransform11.fileContainsInitClass = destFile
                }
            }
            file.close()
        }
    ...
    static boolean shouldProcessClass(String entryName) {
        return entryName != null && entryName.startsWith(ScanSetting11.ROUTER_CLASS_PACKAGE_NAME)
    }
    ...
    static void scanClass(InputStream inputStream, String filePath) {
        if (!filePath.endsWith(".class")) {
            inputStream.close()
            println("HSBC LAB Arouter >>> scan class warning,the file is not a class:" + filePath)
            return
        }
    ...
    static class ScanClassVisitor extends ClassVisitor {
        ...
        void visit(int version, int access, String name, String signature,
            ...
            RegisterTransform11.registerList.each { ext ->
